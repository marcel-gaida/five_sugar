...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === "CRL_OPEN_POPUP") {
    // Safe fallback: open the extension options/popup page as a new tab
...
async function fetchRatings(companyName) {
  const results = {
    ...
    try {
      const gmData = await fetchGoogleMaps(companyName);
...
function scrapeViaTab(url, scrapeScriptFn, timeoutMs = 8000) {
...
async function fetchGoogleMaps(company) {
  ...
  if (!googleApiKey) {
    return fetchGoogleMapsNoKey(company);
  }
  ...
  try {
    ...
    if (!data.results?.length) {
      ...
      const fallbackUrl = `https://maps.googleapis.com/maps/api/place/textsearch/json`
        + `?query=${encodeURIComponent(company + " reviews")}`
        + `&key=${googleApiKey}`;
        
      const res2 = await fetch(fallbackUrl);
      const data2 = await res2.json();
      
      if (!data2.results?.length) return fetchGoogleMapsNoKey(company);
      return parseGoogleResult(data2.results[0]);
    }
...
async function fetchGoogleMapsNoKey(company) {
